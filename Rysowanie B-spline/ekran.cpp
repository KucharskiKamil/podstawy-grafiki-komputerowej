...
void Ekran::bsplajn(vector<vector<int>> vect, int k)
{
    if(vect.size()>3)
    {
        ...
        for(int i=0;i<vect.size()-3;i++)
        {
            for(int j=0;j<=k;j++)
            {
                ...
                punktX=(((-t*t*t+3*t*t-3*t+1)/6)*vect[i][0])+(((3*t*t*t-6*t*t+4)/6)*vect[i+1][0])+(((-3*t*t*t+3*t*t+3*t+1)/6)*vect[i+2][0])+(((t*t*t)/6)*vect[i+3][0]);
                punktY=(((-t*t*t+3*t*t-3*t+1)/6)*vect[i][1])+(((3*t*t*t-6*t*t+4)/6)*vect[i+1][1])+(((-3*t*t*t+3*t*t+3*t+1)/6)*vect[i+2][1])+(((t*t*t)/6)*vect[i+3][1]);
                if(j==0)
                {
                    poprzedniX=punktX;
                    poprzedniY=punktY;
                ...
                {
                    qDebug("Powinno narysowac punkt x:%d y:%d %f",punktX,punktY,t);
